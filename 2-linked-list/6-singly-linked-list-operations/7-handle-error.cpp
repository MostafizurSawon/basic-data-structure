...
void insert_at_any_position(Node *head, int pos, int val)
{
  ...
  for(int i=1; i<pos; i++)
  {
    ...
    if(tmp==NULL)
    {
      cout<<endl<<"Invalid Index. Please try again."<<endl<<endl;
      return;
    }
  }
...
void delete_from_position(Node *head, int pos)
{
  if(head==NULL)
  {
    cout<<endl<<"Linked list is empty. try inserting some values first!"<<endl<<endl;
    return;
  }
  Node *tmp = head;
  for(int i=1;i<pos;i++)
  ...
  delete deleteNode;
  cout<<"Deleted at position "<<pos<<" successfully!"<<endl<<endl;
}
...
void delete_head(Node *&head)
{
  if(head==NULL)
    {
      cout<<endl<<"Linked list is empty. Try to insert some value first!"<<endl<<endl;
      return;
    }
  Node *tmp = head;
  head=head->next;
  delete tmp;
  cout<<endl<<"Deleted head successfully!"<<endl;
}
